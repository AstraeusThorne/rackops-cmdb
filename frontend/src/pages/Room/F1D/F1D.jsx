...
import styles from './F1D.module.css';
...
const F1D = () => {
  ...
  // 加载机房数据：先取机房列表定位 F1D，再一次请求获取机柜及设备数，最后异步拉取 PDU 状态
  useEffect(() => {
    const fetchRoomData = async () => {
      try {
        ...
        if (!Array.isArray(rooms)) {
          message.error('获取机房列表失败：数据格式错误');
          setLoading(false);
          return;
        }

        const f1dRoom = rooms.find((room) => room.name === 'F1D');
        if (!f1dRoom) {
          message.error('未找到F1D机房');
          setLoading(false);
          return;
        }

        const roomId = f1dRoom.id;
        ...
        const uniqueById = Array.from(new Map(cabinetsWithPosition.map((c) => [c.id, c])).values());
        const sortedCabinets = [...uniqueById].sort((a, b) => {
    ...
    <div className={styles.roomPage}>
      ...
      <div className={styles.pageHeader}>
        <div className={styles.headerContent}>
          <h2 className={styles.pageTitle}>F1D机房</h2>
          {roomData && (
